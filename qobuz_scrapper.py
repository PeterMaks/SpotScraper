...
def is_track_match(target_track, video_title):
    """Simple track title match using normalized containment."""
    if not target_track:
        return True
    ...
    if not norm_target or not norm_title:
        return True
    # Check if target track is contained in the video title or vice versa
    if norm_target in norm_title or norm_title in norm_target:
        return True
    # Fallback: fuzzy match
    import difflib
    similarity = difflib.SequenceMatcher(None, norm_target, norm_title).ratio()
...
def is_duration_match(expected_duration_str, video_duration_seconds, tolerance=45):
    ...
    try:
        ...
        if len(parts) == 2:
            ...
            return True
        return abs(expected_seconds - video_duration_seconds) <= tolerance
    except Exception:
        return True
                ...
                if selected_entry:
                    ...
                    if new_file:
                        persistent_cache[item] = {
                            ...
                            "query": item,
                            "file_path": new_file,
                            "youtube_title": item_dict.get("track", os.path.splitext(os.path.basename(new_file))[0]),
                            "youtube_artist": item_dict.get("artist", title)
                        }
                        save_cache(persistent_cache)
                    ...
                    if is_single_query:
                        ...
                        if fallback_entry:
                            ...
                            print(f"  ✓ [Fallback] Found Match: {title}")
                            if playlist_name:
                                print(f"  Folder: downloads/{playlist_name}")
                            print(f"  -> Downloading at 320kbps MP3 (with metadata & cover art)...")
                            ...
                            yt_duration = fallback_entry.get('duration', 0)
                            mins, secs = divmod(int(yt_duration), 60)
                            duration_str = f"{mins}m {secs}s" if yt_duration else '-'
                            
                            print(f"✅ Download Triggered for: {item} (via release-date fallback)")
